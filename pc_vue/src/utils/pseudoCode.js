...
export const listHtml = () => {
  ...
  const headerDiv = new Vue()
  headerDiv.functional = true
  headerDiv.render = (h, context) => {
    return (
      <p class={["topP", { old: context.props.styleItem === 'old' }]} {...{ on: { click: context.listeners.click2 } }}></p>
    )
  }
  ...
  export const onInput = debounce(function () {
    this.getba()
  }, 500)

  // jsx format数据
  export const formatCol = () => {
    this.table.col = this.table.col.map(item => {
      if (item.name === 'changed') {
        item.formatter = (row, column, cellValue) => {
          return (<span>{cellValue === '1' ? '是' : '否'}</span>)
        }
      }
      else if (item.name === 'operate') {
        item.formatter = (row, column, cellValue) => {
          return (<el-button type="text" disabled={row.changed === '1'}>替换</el-button>)
        }
      }
      return item
    })
  ...
  export const handleCtrol = (value, filed) => {
    // 多控
    if (filed.ctrol) {
      // 多控数组循环
