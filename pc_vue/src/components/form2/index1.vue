<template>
  <div>
    ...
    <Form v-show="formData.custType === '01'"
          :model="formData"
          :rules="ruleValidate">
      <div class="formlist">
        <ul>
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.perTypeMap.certTpMarketing"
                        placeholder="请选择"
                        class="sel"
                        disabled>
                  <option v-for="item in mSelectData.CLPM_PERSCUST_CERT_TP_CD"
                          :value="item.key"
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.perTypeMap.sex"
                        placeholder="请选择"
                        class="sel">
                  <option v-for="item in mSelectData.CLPM_SEX_CODE"
                          :value="item.key"
                          :key="item.key">{{item.value}}
                  </option>
                </select>
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.perTypeMap.eduBgdCode"
                        placeholder="请选择"
                        class="sel">
                  <option v-for="item in mSelectData.CLPM_EDU_BGD_CODE"
                          :value="item.key"
                          :key="item.key">{{item.value}}
                  </option>
                </select>
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.perTypeMap.marrgSituCode"
                        placeholder="请选择"
                        class="sel">
                  <option v-for="item in mSelectData.MARRG_SITU_CODE"
                          :value="item.key"
                          :key="item.key">{{item.value}}
                  </option>
                </select>
          ...
          <li v-if="formData.marrgSituCode === '1'">
            <label>配偶姓名：</label>
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <input type="text"
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <textarea class="areatxt"
                          v-model="formData.perTypeMap.mainWorkresm"
                          oninput="value=value.replace(/[`~^\\^'\\\\\\/~`<>]/g,'')"></textarea>
              </div>
            </FormItem>
          </li>
        </ul>
    ...
    <Form v-show="formData.custType === '02'"
          :model="formData"
          :rules="ruleValidate">
      <div class="formlist">
        <ul>
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.lglerTypeMap.corpSizeCodeMarketing"
                        placeholder="请选择"
                        class="sel">
                  <option v-for="item in mSelectData.CLPM_CORP_SIZE_CODE"
                          :value="item.key"
                          :key="item.key">{{item.value}}
                  </option>
                </select>
          ...
          <li>
            <label>所属行业：</label>
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright white"
                   @click="showSwitch1(true)">
                <input type="text"
                       class="txt"
          ...
          <li>
            ...
            <FormItem prop="majorBusiOprmngAddr">
              <div class="formright">
                <select v-model="formData.lglerTypeMap.icbStaQxb"
                        placeholder="请选择"
                        class="sel">
                  <option v-for="item in mSelectData.ICB_STA_QXB"
                          :value="item.key"
                          :key="item.key">{{item.value}}
                  </option>
                </select>
      ...
      <form-self :formList="formList"
        ...
        <template #holdingInfos>
          <template v-for="(item, index) in form.holdingInfos">
            <holdingInfo-self :formList="holdingFormListFilter(item)"
                              :form="item"
                              :index="index"
                              :rules="rules"
                              :key="index"
                              formStr="holdingInfos"
                              @formChangeListen="holdingInfoFormChange($event, index)"
                              @inputListen="holdingInfoInputListen($event, index)"
                              @btnListen="holdingInfoBtnListen($event, index)"></holdingInfo-self>
          </template>
        </template>
        <template #personInfoArray>
          <template v-for="(item, index) in form.personInfoArray">
            <holdingInfo-self :formList="personFormListFilter(item)"
                              :form="item"
                              :index="index"
                              :rules="rules"
                              :key="index"
                              formStr="personInfoArray"
                              @formChangeListen="personInfoFormChange($event, index)"
                              @inputListen="personInfoInputListen($event, index)"
                              @btnListen="personInfoBtnListen($event, index)"></holdingInfo-self>
          </template>
        </template>
        <template #earningsInfoArray>
          <template v-for="(item, index) in form.earningsInfoArray">
            <holdingInfo-self :formList="earningsFormListFilter(item)"
                              :form="item"
                              :index="index"
                              :rules="rules"
                              :key="index"
                              formStr="earningsInfoArray"
                              @formChangeListen="earningsInfoFormChange($event, index)"
                              @inputListen="earningsInfoInputListen($event, index)"
                              @btnListen="earningsInfoBtnListen($event, index)"></holdingInfo-self>
          </template>
        </template>
      </form-self>
    </div>
    <div class="btngroup"
         v-show="formData.custType === '01'">
      <a @click="syncEnterpreneurInfo()"
         class="sqlbtn">同步</a>
    </div>
    <div class="btngroup"
         v-show="formData.custType === '02'">
      <a @click="syncEnterpreneurInfo2()"
         class="sqlbtn">同步</a>
    </div>
    <div class="subtop">
    ...
    <div class="tablelist unify_table_css">
      <table border="0"
        ...
        <tr v-for="(item,index) in formData.corpBaseArray"
            :key="item.key">
          <td>
            <select v-model="item.personType"
                    class="sel">
              <option v-for="(item1,index) in mSelectData.PERSON_TYPE"
                      :key="index"
                      :value="item1.key">{{item1.value}}
              </option>
            </select>
          </td>
          <td>
          ...
          <td>
            <select v-model="item.certTp"
                    class="sel">
              <option v-for="(item1,index) in mSelectData.CLPM_PERSCUST_CERT_TP_CD"
                      :key="index"
                      :value="item1.key">{{item1.value}}
              </option>
            </select>
    ...
    <div class="tablelist unify_table_css">
      <table border="0"
        ...
        <tr v-for="(item,index) in formData.corpPostArray"
            :key="item.key">
          <td>
            <select v-model="item.postn"
                    class="sel">
              <option v-for="(item1,index) in mSelectData.POSTN"
                      :key="index"
                      :value="item1.key">{{item1.value}}
              </option>
            </select>
    ...
    <div class="formlist">
      <ul>
        ...
        <!--        </li>-->
        <li class="longli"
            v-if="formData.custType === '02'">
          <label>在我行授信情况：</label>
          <div class="formright">
            <textarea class="areatxt"
                      v-model="formData.lglerTypeMap.remark"></textarea>
          </div>
        </li>
      </ul>
    </div>
    <div class="btngroup">
      <a @click="onSave('tempSave')"
         class="sqlbtn"
         v-if="toPublicInfo.busiStatus === '10'">暂存</a>
      <a @click="onSave('save')"
         class="sqlbtn"
         v-else>保存</a>
    </div>
    ...
    <div class="dialog"
      ...
      <div class="treeList">
        <Tree ref="tree1"
              ...
              @on-select-change="selectedChang"></Tree>
      </div>
    </div>
...
import {
  enterpreneurFormList, holdingFormList, personFormList, earningsFormList,
  holdingInfo, personInfo, earningsInfo, dateObj
} from '../staticData'
import {
  telValidator, nameValidator, cardTypeValidator, startDateValidator,
  endDateValidator, percentValidator, nationValidator, emptyValidator
} from '../validator'
import {
  oprtorInfoTpCd as oprtorInfoTpCdOption,
  lglerCertType as lglerCertTypeOption, selSerChanlNo as selSerChanlNoOption
...
export default {
  ...
  methods: {
    initDBData () {
      const str = ['CLPM_CORP_SIZE_CODE', 'CLPM_UNIT_BELONG_INDS_TYPE', 'CLPM_PERSCUST_CERT_TP_CD',
        'CLPM_SEX_CODE', 'CLPM_PERSCUST_CERT_TP_CD', 'CLPM_EDU_BGD_CODE']
      const str2 = ['ICB_STA_QXB', 'MARRG_SITU_CODE', 'PERSON_TYPE', 'POSTN']
      NativeLoanDB.getDics(str).then((value) => {
        this.mSelectData = value
        this.mSelectData = { ...this.mSelectData, ...getDisc(str2) }
      })
    ...
    // 控股股东/实际控制人员信息 数量改变
    holdingInfosListen () {
      const start = this.form.holdingInfos.length
      const end = Number(this.form.holdingsCount)
      const add = Math.abs(end - start)
    ...
    personInfoArrayListen () {
      const start = this.form.personInfoArray.length
      const end = Number(this.form.naturalPerNum)
      const add = Math.abs(end - start)
      if (start < end) { // 新增关联自然人的数量
        for (let i = 0; i < add; i++) {
    ...
    earningsInfoArrayListen () {
      ...
      const start = this.form.earningsInfoArray.length
      const end = Number(this.form.btnPerNum)
      const add = Math.abs(end - start)
      if (start < end) { // 新增的数量
        for (let i = 0; i < add; i++) {
