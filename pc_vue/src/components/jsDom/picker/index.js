...
const instance = (content) => {
  return new Promise((resolve, reject) => {
    ...
    instanceDom.onConfirm = (value, index) => {
      // type 定义事件类型eg:confirm
      resolve({ type: 'confirm', value, index }) // 只能接收一个参数
    ...
    // change不能多次调用resolve
    instanceDom.onChange = (picker, value, index) => {
      // type 定义事件类型eg:confirm
      resolve({ type: 'change', value, index }) // 只能接收一个参数
    }
